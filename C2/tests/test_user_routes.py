...
class TestUserRoutesAPI(unittest.TestCase):
    ...
    def test_user_flow(self):
        # Login
        res = self.client.post("/api/v1/login", json={"username": "admin", "password": "adminpass"})
        ...
        # Get user info
        res = self.client.get("/api/v1/user/userinfobyid", params={"userid": 2})
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.json()["username"], "newuser")

        # Delete user
        res = self.client.delete("/api/v1/user/delate", params={"userid": 2}, headers=headers)
        self.assertEqual(res.status_code, 200)
